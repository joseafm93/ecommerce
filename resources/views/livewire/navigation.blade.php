    ...
    <div class="container-menu flex items-center h-16 justify-between md:justify-start">
        <a :class="{'bg-opacity-100 text-orange-500': open}"
           ...
           class="flex flex-col items-center justify-center order-last md:order-first px-6 sm:px-4 bg-white bg-opacity-25 text-white cursor-pointer font-semibold h-full">
            ...
            <span class="showCategory text-sm hidden sm:block">
                Categorías
            </span>
        </a>

        <a href="/" class="mx-6">
            <x-jet-application-mark dusk="homePage" class="block h-9 w-auto"></x-jet-application-mark>
        </a>
        ...
        <div dusk="dropdownCart" class="hidden md:block">
                ...
                <ul class="bg-white">
                    @foreach($categories as $category)
                        <li class="navigation-link text-trueGray-500 hover:bg-orange-500 hover:text-white">
                            <a href="{{ route('categories.show', $category) }}" class="category-{{ $category->id }} py-2 px-4 text-sm flex items-center">
