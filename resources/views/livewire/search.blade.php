<div class="flex-1 relative">
    <form action="{{ route('search') }}" autocomplete="off" method="get">
        <x-jet-input dusk="searchInput" name="name" wire:model="search" type="text" class="flex w-full" placeholder="¿Estás buscando algún producto?"></x-jet-input>
        ...
        <div class="bg-white rounded-lg shadow-lg">
            <div class="px-4 py-3 space-y-1">
                @forelse ($products as $product)
                    <a dusk="product-{{ $product->name }}" href="{{ route('products.show', $product) }}" class="flex">
